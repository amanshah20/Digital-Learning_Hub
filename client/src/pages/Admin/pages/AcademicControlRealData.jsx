...
const AcademicControl = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingSubject) {
        await api.put(`/admin/courses/${editingSubject.id}`, formData);
    ...
    <div className="space-y-6">
      <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="glass-card p-6 lg:p-8">
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
          ...
          <button onClick={() => {setShowModal(true); setEditingSubject(null);}} className="bg-primary-500 text-white px-6 py-3 rounded-lg hover:bg-primary-600 transition-all font-medium flex items-center gap-2 w-full lg:w-auto justify-center">
      ...
      <div className="glass-card p-2 flex gap-2">
        {['subjects', 'semesters', 'curriculum'].map(tab => (
          <button key={tab} onClick={() => setActiveTab(tab)} className={`px-4 py-2 rounded-lg font-medium transition-all ${activeTab === tab ? 'bg-primary-500 text-white' : 'hover:bg-white/30 dark:hover:bg-slate-800/30'}`}>
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="glass-card p-4">
        <div className="relative">
          ...
          <input type="text" placeholder="Search subjects..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full pl-10 pr-4 py-2 rounded-lg border border-slate-200 dark:border-slate-700 bg-white/50 dark:bg-slate-800/50 focus:outline-none focus:ring-2 focus:ring-primary-500" />
        </div>
      </motion.div>
      ...
      {activeTab === 'subjects' && (
        <div className="glass-card p-6 overflow-x-auto">
          {loading ? (
            <div className="flex justify-center py-8"><div>Loading subjects...</div></div>
          ) : filteredSubjects.length === 0 ? (
            <div className="flex justify-center py-8"><div>No subjects found</div></div>
          ) : (
                  ...
                  <tr key={s.id} className="border-b border-slate-200 dark:border-slate-700 hover:bg-white/30 dark:hover:bg-slate-800/30">
                    ...
                    <td className="p-3">
                      <div className="flex justify-end gap-2">
                        <button onClick={() => {setEditingSubject(s); setFormData({name: s.name, code: s.code, credits: s.credits || 3, semester: s.semester || 1, teacher: ''}); setShowModal(true);}} className="p-2 hover:bg-blue-500/10 rounded-lg text-blue-600">
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="glass-card p-6 max-w-2xl w-full">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <input type="text" placeholder="Teacher Name" value={formData.teacher} onChange={(e) => setFormData({...formData, teacher: e.target.value})} className="w-full px-4 py-2 rounded-lg border border-slate-200 dark:border-slate-700 bg-white/50 dark:bg-slate-800/50 focus:outline-none focus:ring-2 focus:ring-primary-500" />
